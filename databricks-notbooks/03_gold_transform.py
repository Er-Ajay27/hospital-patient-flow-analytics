...
incoming_patient = (silver_df.select(
    "patient_id",
    "gender",
    "age")
...
#Create an expression to detect attribute changes (hash or explicit comparisions)
#We'll use simple concat hash to detect changes
...
#Build insert DF: Join incoming with target to figure new inserts where either not exist or changed 
insert_df = spark.sql("""
...
if insert_df.count() > 0:
    insert_df.write.format("delta").mode("append").save(gold_dim_patient)


#Department dimension table Creation

#Prepare incoming (latest per patient feed snapshot)
incoming_dept = (silver_df.select(
    "department",
    "hospital_id"))
...
#Read current Dim (fillter is_current =true)
...
fact_base = (silver_df
             ...
             .withColumn("admission_date", F.to_date(col("admission_time")))
            )

#Join with dim to get surrogate keys
...
# Let's make columns names explicit instead: 
...
#ADLS Configuration
spark.conf.set(
  "fs.azure.account.key.hospitalstoreg.dfs.core.windows.net",
  dbutils.secrets.get(scope = "hospitalanalyticsvaultscope", key = "storage-connection")
)

gold_dim_patient = "abfss://gold@hospitalstoreg.dfs.core.windows.net/dim_patient"

display(spark.read.format("delta").load(gold_dim_patient).count())